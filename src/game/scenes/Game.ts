...
export class Game extends Scene {
  ...
  private createHUD(): void {
    ...
    this.livesText = this.add.text(20, 50, `LIVES: ${this.lives}`, textStyle);
    ...
    this.weaponNameText.setOrigin(1, 1);
  ...
  private updateHUD(): void {
    ...
    this.livesText.setText(`LIVES: ${this.cheatMode ? "∞" : this.lives}`);
  ...
  private missileHitAsteroid(
    ...
    this.particles.explode(30, explosion.x, explosion.y);

    // Draw explosion circle briefly
    ...
    explosionGraphics.lineStyle(3, 0xffaa00, 1);
    explosionGraphics.strokeCircle(explosion.x, explosion.y, explosion.radius);
  ...
  private explodeAllMissiles(): void {
    ...
    missiles.forEach((missile) => {
      ...
      if (explosion) {
        ...
        this.particles.explode(15, explosion.x, explosion.y);
  ...
  private updateCooldownCircle(): void {
    ...
    const centerY = this.cameras.main.height - 30;
    const radius = 16;

    // Background circle (dark)
    this.cooldownCircle.lineStyle(3, 0x333333, 0.8);
    this.cooldownCircle.strokeCircle(centerX, centerY, radius);

    // Progress arc (colored based on weapon)
    const weapon = this.weaponManager.getCurrentWeapon();
    let color = 0x00ff00; // Blaster green
    if (weapon.name === "LASER") color = 0xff0000;
    else if (weapon.name === "MISSILE") color = 0xff8800;

    this.cooldownCircle.lineStyle(3, color, 1);
